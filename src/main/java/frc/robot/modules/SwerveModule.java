...
public class SwerveModule {
    ...
    public SwerveModule(int steer_id, int drive_id, int angle_id) {
        ...
        angle_controller.enableContinuousInput(0, 360);
    }
    ...
    public void set_module_state(SwerveModuleState state) {
        ...
        double steer_output = angle_controller.calculate(get_raw_angle(), state.angle.getDegrees());
    ...
    public double get_raw_angle() {
        return steer_cancoder.getAbsolutePosition().getValue() * 360;
    }

    public SwerveModulePosition get_module_position() {
        return new SwerveModulePosition(drive_encoder.getPosition(),
                Rotation2d.fromDegrees(this.get_raw_angle()));
    }
